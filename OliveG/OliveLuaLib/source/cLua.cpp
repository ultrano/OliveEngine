...
cLua::cLua(lua_State* pWrapState)
{
	m_pScriptContext = pWrapState;
}
cLua::~cLua()
{
	if(m_pScriptContext)
...
void* cLua::GetUserDataArgument(int num, void* dDefault)
{
	return lua_isnoneornil(m_pScriptContext,num)? (dDefault):luaL_checkudata(m_pScriptContext,num,"userdata");
}
...
void cLua::PushUserData(void* pUserData)
{
	lua_pushlightuserdata(m_pScriptContext,pUserData);
}